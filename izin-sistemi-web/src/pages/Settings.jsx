...
export default function Settings() {
    ...
    const fetchBirimler = async () => {
        const token = localStorage.getItem('token');
        try {
            const res = await axios.get('http://localhost:5000/api/personel/birimler', { headers: { Authorization: `Bearer ${token}` } });
    ...
    const createUser = async (e) => {
        ...
        try {
            ...
            await axios.post('http://localhost:5000/api/auth/register', newUser, { headers: { Authorization: `Bearer ${token}` } });
    ...
    const transferEt = async () => {
        ...
        const token = localStorage.getItem('token');
        try {
            await axios.post('http://localhost:5000/api/personel/transfer', {
            ...
            if (selectedRol && selectedRol !== editingUser.rol_adi && user.rol === 'admin') {
                await axios.post('http://localhost:5000/api/personel/rol-degistir', {
                    personel_id: editingUser.personel_id,
    ...
    const personelDondur = async (neden) => {
        const token = localStorage.getItem('token');
        try {
            await axios.post('http://localhost:5000/api/personel/dondur', {
    ...
    const personelAktifEt = async (id) => {
        ...
        const token = localStorage.getItem('token');
        try {
            await axios.post('http://localhost:5000/api/personel/aktif-et', { personel_id: id }, { headers: { Authorization: `Bearer ${token}` } });
    ...
    const personelSil = async (id) => {
        ...
        const token = localStorage.getItem('token');
        try {
            await axios.delete(`http://localhost:5000/api/personel/sil/${id}`, { headers: { Authorization: `Bearer ${token}` } });
    ...
    const updatePassword = async (e) => {
        ...
        const token = localStorage.getItem('token');
        try {
            await axios.post('http://localhost:5000/api/personel/guncelle', { yeni_sifre: yeniSifre }, { headers: { Authorization: `Bearer ${token}` } });
