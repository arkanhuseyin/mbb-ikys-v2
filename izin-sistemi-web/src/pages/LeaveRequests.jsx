...
export default function LeaveRequests() {
    ...
    const verileriGetir = async (token) => {
        try {
            const response = await axios.get('http://localhost:5000/api/izin/listele', { headers: { Authorization: `Bearer ${token}` } });
    ...
    const incele = async (izin) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const res = await axios.get(`http://localhost:5000/api/izin/timeline/${izin.talep_id}`, { headers: { Authorization: `Bearer ${token}` } });
    ...
    const onayla = async (onayTuru) => {
        ...
        try {
            await axios.post('http://localhost:5000/api/izin/onayla', { talep_id: secilenTalep.talep_id, imza_data: imzaResmi, yeni_durum: yeniDurum }, { headers: { Authorization: `Bearer ${token}` } });
    ...
    const reddet = async () => {
        ...
        const token = localStorage.getItem('token');
        try {
            await axios.post('http://localhost:5000/api/izin/onayla', { talep_id: secilenTalep.talep_id, imza_data: null, yeni_durum: 'REDDEDILDI' }, { headers: { Authorization: `Bearer ${token}` } });
    ...
    const islakImzaIslemi = async (durum) => {
        ...
        try {
            await axios.post('http://localhost:5000/api/izin/islak-imza-durumu', { 
    ...
    const downloadForm2 = () => {
        ...
        const url = `http://localhost:5000/api/izin/pdf/form2/${secilenTalep.talep_id}?hrName=${hrName}&managerName=${printSettings.managerName}&managerTitle=${managerTitle}&headName=${printSettings.headName}&headTitle=${headTitle}`;
    ...
    const getBelgeUrl = (yol) => {
        ...
        return `http://localhost:5000/uploads/izinler/${dosyaAdi}`;
    };
        ...
        <div className="container-fluid p-4">
            ...
            {secilenTalep && (<div className="modal d-block" style={{backgroundColor: 'rgba(0,0,0,0.5)', zIndex:1050}}><div className="modal-dialog modal-dialog-centered modal-lg"><div className="modal-content border-0 shadow-lg rounded-4">
                ...
                <div className="modal-footer bg-light border-top p-3"><div className="d-flex justify-content-between w-100 align-items-center"><div className="d-flex gap-2">
                    <a href={`http://localhost:5000/api/izin/pdf/form1/${secilenTalep.talep_id}`} target="_blank" className="btn btn-outline-dark btn-sm fw-bold"><Printer size={16} className="me-1"/> Form 1</a>
