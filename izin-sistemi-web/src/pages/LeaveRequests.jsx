...
export default function LeaveRequests() {
    ...
    const verileriGetir = async (token) => {
        try {
            // GÜNCEL URL
            const response = await axios.get('https://mbb-ikys-v2.onrender.com/api/izin/listele', { headers: { Authorization: `Bearer ${token}` } });
    ...
    const incele = async (izin) => {
        ...
        try {
            const token = localStorage.getItem('token');
            // GÜNCEL URL
            const res = await axios.get(`https://mbb-ikys-v2.onrender.com/api/izin/timeline/${izin.talep_id}`, { headers: { Authorization: `Bearer ${token}` } });
    ...
    const onayla = async (onayTuru) => {
        ...
        try {
            // GÜNCEL URL
            await axios.post('https://mbb-ikys-v2.onrender.com/api/izin/onayla', { talep_id: secilenTalep.talep_id, imza_data: imzaResmi, yeni_durum: yeniDurum }, { headers: { Authorization: `Bearer ${token}` } });
    ...
    const reddet = async () => {
        ...
        try {
            // GÜNCEL URL
            await axios.post('https://mbb-ikys-v2.onrender.com/api/izin/onayla', { talep_id: secilenTalep.talep_id, imza_data: null, yeni_durum: 'REDDEDILDI' }, { headers: { Authorization: `Bearer ${token}` } });
    ...
    const islakImzaIslemi = async (durum) => {
        ...
        try {
            // GÜNCEL URL
            await axios.post('https://mbb-ikys-v2.onrender.com/api/izin/islak-imza-durumu', { 
    ...
    const downloadForm2 = () => {
        ...
        // GÜNCEL URL
        const url = `https://mbb-ikys-v2.onrender.com/api/izin/pdf/form2/${secilenTalep.talep_id}?hrName=${hrName}&managerName=${printSettings.managerName}&managerTitle=${managerTitle}&headName=${printSettings.headName}&headTitle=${headTitle}`;
    ...
    const getBelgeUrl = (yol) => {
        ...
        // GÜNCEL URL
        return `https://mbb-ikys-v2.onrender.com/uploads/izinler/${dosyaAdi}`;
    };
        ...
        <div className="container-fluid p-4">
            ...
            {secilenTalep && (<div className="modal d-block" style={{backgroundColor: 'rgba(0,0,0,0.5)', zIndex:1050}}><div className="modal-dialog modal-dialog-centered modal-lg"><div className="modal-content border-0 shadow-lg rounded-4">
                ...
                <div className="modal-footer bg-light border-top p-3"><div className="d-flex justify-content-between w-100 align-items-center"><div className="d-flex gap-2">
                    {/* GÜNCEL URL */}
                    <a href={`https://mbb-ikys-v2.onrender.com/api/izin/pdf/form1/${secilenTalep.talep_id}`} target="_blank" className="btn btn-outline-dark btn-sm fw-bold"><Printer size={16} className="me-1"/> Form 1</a>
